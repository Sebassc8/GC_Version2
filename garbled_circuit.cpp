...
int GarbleStr(const string& scd_file_address, const string& p_init_str,
              ...
              string* output_str,string* output_str_c) {
  if (clock_cycles == 0) {
    return FAILURE;
  }
 ...
 BIGNUM* p_init_c = BN_new();
  BIGNUM* p_input_c = BN_new();
...
//cout<<"Bob input str: "<<input_str_bob<<endl;
 ...
 key_t key_inl = ftok("shmfile",65); 
key_t key_inpl = ftok("shmfile",64); 

// shmget returns an identifier in shmid
int shmid_inl = shmget(key_inl,1024,0666|IPC_CREAT);
int shmid_inpl = shmget(key_inpl,1024,0666|IPC_CREAT);

    // shmat to attach to shared memory
init_labels = (block*) shmat(shmid_inl,(void*)0,0);
input_labels = (block*) shmat(shmid_inpl,(void*)0,0);
  ...
  block* init_labels_c = (block*) shmat(shmid_inl,(void*)0,0);
  block* input_labels_c = (block*) shmat(shmid_inpl,(void*)0,0);
  block* output_labels_c = nullptr;
  short* output_vals_c = nullptr;



  CHECK(
      EvaluateMakeLabels(garbled_circuitb, &init_labels_c, &input_labels_c,
                         &output_labels_c, &output_vals_c, clock_cycles));                   
...
// g_input
  for (uint cid = 0; cid < clock_cycles; cid++) {
    for (uint i = 0; i < garbled_circuit.g_input_size; i++) {
     ...
     input_labels_c[(cid * garbled_circuitb.get_secret_input_size() + i)]=input_labels[(cid * garbled_circuit.get_secret_input_size() + i)
                    * 2 + 0];
...
BitTag Alice_taggedmsg[message_len][2]; 
//      cout<<"msg lengh"<<message_len<<endl;
      ...
      for (uint j = 0; j < 2; j++) {
       message[idx][j] = input_labels[(cid
            * garbled_circuit.get_secret_input_size() + i
            ...
            Alice_taggedmsg[idx][j] = {{message[idx][j]},all_one_block};
      }
    }
  }
//PRG pg;
...
for(int64_t i = 0; i < length; ++i)
 ...
 if (message != nullptr) {
    ...
    delete[] message;
        ...
        BitTag Bob_taggedmsg[message_len]; 

  for (uint i = 0; i < garbled_circuitb.e_init_size; i++) {

    init_labels_c[i + garbled_circuitb.g_init_size] = init_labels[i];
  }
  for (uint cid = 0; cid < clock_cycles; cid++) {
    for (uint i = 0; i < garbled_circuitb.e_input_size; i++) {
      uint indx = garbled_circuitb.e_init_size
        ...
        Bob_taggedmsg[indx] = {{rmessage[indx]},zero_block}; //Bob's tagged message with bit 0
    }
  }
...
DUMP("r_key") << R << endl;
  ...
  CHECK_ALLOC(garbled_tables_temp = new GarbledTable[garbled_tables_size]);
   ...
   key_t key_gt = ftok("shmfile",66); 
// shmget returns an identifier in shmid
int shmid_gt = shmget(key_gt,garbled_tables_size,0666|IPC_CREAT);
    // shmat to attach to shared memory
  GarbledTable* garbled_tables = (GarbledTable*) shmat(shmid_gt,(void*)0,0);
...
//If Alice bit tag is 1
 if (((cmpBlock(&Alice_taggedmsg[message_len][0].bit_tag,&all_one_block,0))==1)&&(cmpBlock(&Alice_taggedmsg[message_len][1].bit_tag,&all_one_block,0))==1)
{
  ...
  for (uint64_t cid = 0; cid < (clock_cycles); cid++) {
   ...
   std::cout << "Garbling time (cc) = " << garble_time << endl;
  }
  ...
  for (uint cid = 0; cid < clock_cycles; cid++) {
    for (uint i = 0; i < garbled_circuitb.e_input_size; i++) {
      ...
      input_labels_c[(cid * garbled_circuitb.get_secret_input_size() + i
          + garbled_circuitb.g_input_size)] = Bob_taggedmsg[indx].message;
    }
  }
 ...
 block *wires_bob = nullptr;
  ...
  GarbledTable* garbled_tablesb = (GarbledTable*) shmat(shmid_gt,(void*)0,0);
...
//garbled_tablesb=garbled_tables;
   ...
   if ((cmpBlock(&Bob_taggedmsg[message_len].bit_tag,&zero_block,0))==1) //IF BOB's bit tag is 0
   {
    ...
    for (uint64_t i = 0; i < garbled_circuitb.gate_size; i++) {  // secret values
      ...
      if (fanoutb[i] > 0 && IsSecret(wires_valb[output])) {
        ...
        EvalGate(input0_labels, input0_value, input1_labels, input1_value, type,
                 ...
                 &wires_bob[output], &wires_valb[output]); //Evaluation after garbling and checking bit TAGs
    ...
    for (uint64_t i = 0; i < garbled_circuitb.output_size; i++) {
      output_labels_c[cid * garbled_circuitb.output_size + i] =
          wires_bob[garbled_circuitb.outputs[i]];
      output_vals_c[cid * garbled_circuitb.output_size + i] =
          wires_valb[garbled_circuitb.outputs[i]];
      DUMP("output") << output_labels_c[cid * garbled_circuitb.output_size + i]
          << endl;
    }
    ...
    for (uint64_t j = 0; j < garbled_table_indb; j++) {  // clear tables
      garbled_tablesb[j].gid = (uint32_t)(-1);
          ...
          cout << "Cloud Terminated in " << cid + 1 << "cc out of "
             << clock_cycles << "cc." << endl;
         clock_cycles = cid + 1;
      
 std::cout << "Cloud evaluation time (cc) = " << eval_time << endl;
  }
 ...
 BIGNUM* output_mask_bnb = BN_new();
  ...
  for (uint64_t cid = 0; cid < clock_cycles; cid++) {
    for (uint64_t i = 0; i < garbled_circuitb.output_size; i++) {
      if (output_vals_c[cid * garbled_circuitb.output_size + i] == 0) {
        BN_clear_bit(output_bn_c, cid * garbled_circuitb.output_size + i);
      } else if (output_vals_c[cid * garbled_circuitb.output_size + i] == 1) {
        if (i >= (uint64_t) BN_num_bits(output_mask_bnb)
            && BN_is_bit_set(output_mask_bnb, i) == 0) {
          BN_set_bit(output_bn_c, cid * garbled_circuitb.output_size + i);
        }
      } else {
short garble_output_type = get_LSB(
            output_labels[(cid * garbled_circuit.output_size + i) * 2 + 0]);
...
delete[] init_labels;
  ...
  delete[] output_vals;
  delete[] init_labels_c;
  delete[] input_labels_c;
  delete[] output_labels_c;
  delete[] output_vals_c;
...
CHECK(  
       OutputBN2StrHighMem(garbled_circuitb, output_bn_c, clock_cycles,
                           output_mode, output_str_c));

    shmdt(init_labels);
    shmdt(input_labels);
    shmdt(garbled_tables);

    shmctl(shmid_inl,IPC_RMID,NULL);
    shmctl(shmid_inpl,IPC_RMID,NULL);
    shmctl(shmid_gt,IPC_RMID,NULL);
